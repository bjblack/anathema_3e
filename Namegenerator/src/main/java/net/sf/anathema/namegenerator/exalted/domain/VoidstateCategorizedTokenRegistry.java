...
public class VoidstateCategorizedTokenRegistry implements ICategorizedTokenConfiguration
{
	...
	private final TokenCategory[] rootTokenCategories = new TokenCategory[]
	{
		...
		new AggregatedTokenCategory ("Tokens.Glues", new TokenCategory[]
		{
			AND, FROM, IN, OF, THE
		}
		)
	...
	public VoidstateCategorizedTokenRegistry ()
	{
		...
		tokensByCategory.put (AND, new String[]
		{
			"and"
		}
		);
		tokensByCategory.put (FROM, new String[]
		{
			"from"
		}
		);
		tokensByCategory.put (IN, new String[]
		{
			"in"
		}
		);
		tokensByCategory.put (OF, new String[]
		{
			"of"
		}
		);
		tokensByCategory.put (THE, new String[]
		{
			"the"
		}
		);
